...
CREATE TABLE IF NOT EXISTS nsk_travel_booking_project_2.analytics.daily_revenue_by_type (
  business_date DATE,
  booking_type STRING,
  total_amount DOUBLE,
...
DELETE FROM nsk_travel_booking_project_2.analytics.daily_revenue_by_type
...
INSERT INTO nsk_travel_booking_project_2.analytics.daily_revenue_by_type (business_date, booking_type, total_amount, total_quantity)
SELECT
...
FROM nsk_travel_booking_project_2.bronze.booking_inc
WHERE business_date = COALESCE(TRY_CAST(:arrival_date AS DATE), current_date())
